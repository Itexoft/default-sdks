 ...
 *
 * Dynamic library loading with pthread-consistent GOT.func indices
 */
  ...
  $loadWebAssemblyModule: (binary, flags, libName, localScope, handle) => {
    ...
    function loadModule() {
      ...
      function postInstantiation(module, instance) {
        ...
        // Input: the side module's exports and element-segment range. Output: GOT.func values
        // containing the same function-table indices on every pthread. updateGOT uses addFunction,
        // whose result can differ between per-thread tables, while __wasm_apply_data_relocs stores
        // that result in shared memory. The element-segment index is identical on every thread.
        for (var symbolName in GOT) {
          var exportedFunction = moduleExports[symbolName];
          if (typeof exportedFunction == 'function') {
            for (var sideModuleTableIndex = tableBase; sideModuleTableIndex < tableBase + metadata.tableSize; sideModuleTableIndex++) {
              if (getWasmTableEntry(sideModuleTableIndex) === exportedFunction) {
                GOT[symbolName].value = sideModuleTableIndex;
                break;
              }
            }
          }
  ...
  $loadDynamicLibrary: function(libName, flags = {global: true, nodelete: true}, localScope, handle) {
    ...
    if (dso) {
      ...
      if (dso.exports === 'loading') {
        throw new Error(`Attempt to load '${libName}' twice before the first load completed`);
    ...
    function discardPendingDSO() {
      if (LDSO.loadedLibsByName[libName] === dso) {
