...
function _setModules(libs) {
    if (!Array.isArray(libs)) {
        throw new Error('runtime config.extensions.extra.itexoftWasmSideModules must be an array');
    }
...
function _setWasmSideModulePaths(paths) {
    if (!paths || typeof paths !== 'object') {
        throw new Error('runtime config.extensions.extra.itexoftWasmSideModulePaths must be an object');
    }
    const entries = Object.entries(paths);
    for (const [name, value] of entries) {
        _validateName(name);
        if (typeof value !== 'string' || value.length === 0) {
            throw new Error('runtime config.extensions.extra.itexoftWasmSideModulePaths values must be non-empty strings');
        }
    }
...
function _applyNativeLibraries(config) {
    ...
    if (!('itexoftWasmSideModules' in extra)) {
        throw new Error('runtime config.extensions.extra.itexoftWasmSideModules is missing');
    }
    if (!('itexoftWasmSideModulePaths' in extra)) {
        throw new Error('runtime config.extensions.extra.itexoftWasmSideModulePaths is missing');
    }
    if (!('itexoftWasmSideModuleAutoLoad' in extra)) {
        throw new Error('runtime config.extensions.extra.itexoftWasmSideModuleAutoLoad is missing');
    }

    _setModules(extra.itexoftWasmSideModules);
    _setWasmSideModulePaths(extra.itexoftWasmSideModulePaths);

    if (!Array.isArray(extra.itexoftWasmSideModuleAutoLoad)) {
        throw new Error('runtime config.extensions.extra.itexoftWasmSideModuleAutoLoad must be an array');
    }
    for (const name of extra.itexoftWasmSideModuleAutoLoad) {
        _validateName(name);
        if (!_state.libs.has(name)) {
            throw new Error(`runtime config.extensions.extra.itexoftWasmSideModuleAutoLoad contains unknown module '${name}'`);
        }
    }
    _state.autoLoad = new Set(extra.itexoftWasmSideModuleAutoLoad);
}
